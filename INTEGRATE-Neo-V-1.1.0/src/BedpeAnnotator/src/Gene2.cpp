...
int Gene::loadGenesFromFile(char* file) {
	...
	while (1) {
		...
		nextN[0]='\0';
		int numnum=sscanf(line_buffer,"%s\t%s\t%s\t%u\t%u\t%u\t%u\t%d\t%s\t%s\t%s\t%s", nameC, chromC, strandC, &txStart, &txEnd, &cdsStart, &cdsEnd, &exonCount, exonStartsC, exonEndsC, tmp11, name2C);
		if(numnum!=12)
		{
			cerr<<"error loading genes at: "<<line_buffer<<endl;
			cerr<<"Make sure this is GenePhred format"<<endl;
                	exit(1);
		}
...
junction_t assign_junction(int gId, int is5p, string chr, int strand, int pos1, int pos2, int exonNum, int is_coding, int baseLeft, string name, int coding_start)
{
...
//cout<<"push baseLeft "<<baseLeft<<endl;
                    juncs.push_back(assign_junction(gid, is5p, chr, strand, pos1, pos2, exonNum, is_coding, baseLeft, name, coding_start));
                }
